...
template<typename Stream>
void Serialize(Stream& s, const CClaimTrieData& u)
{
    Serialize(s, u.hash);
    Serialize(s, u.claims);
    Serialize(s, u.nHeightOfLastTakeover);
}

template<typename Stream>
void Unserialize(Stream& s, CClaimTrieData& u)
{
    Unserialize(s, u.hash);
    if (s.eof()) {
        u.claims.clear();
        u.nHeightOfLastTakeover = 0;
        return;
    }
    Unserialize(s, u.claims);
    Unserialize(s, u.nHeightOfLastTakeover);
}

template<typename Stream>
void Serialize(Stream& s, const CTxOutPointHeightType& u)
{
    Serialize(s, u.outPoint);
    Serialize(s, u.nHeight);
}
...
template<typename Stream>
void Unserialize(Stream& s, CTxOutPointHeightType& u)
{
    Unserialize(s, u.outPoint);
    Unserialize(s, u.nHeight);
}
...
template<typename Stream>
void Serialize(Stream& s, const CNameOutPointType& u)
{
    Serialize(s, u.name);
    Serialize(s, u.outPoint);
}
...
template<typename Stream>
void Unserialize(Stream& s, CNameOutPointType& u)
{
    Unserialize(s, u.name);
    Unserialize(s, u.outPoint);
}
