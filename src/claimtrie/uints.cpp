...
CBaseBlob<BITS>::CBaseBlob() : data(new uint8_t[WIDTH])
{
    SetNull();
}

template<uint32_t BITS>
CBaseBlob<BITS>::CBaseBlob(const std::vector<uint8_t>& vec) : data(new uint8_t[WIDTH])
{
...
template<uint32_t BITS>
CBaseBlob<BITS>::CBaseBlob(const CBaseBlob& o) : data(new uint8_t[WIDTH])
{
    *this = o;
}
